...
internal static class AcceptanceFixture
{
    ...
    public const string AutomatedCheckScript =
        """
        (() => {
          const root = document.getElementById("myco-visual-acceptance");
          ...
          return {
            ...
            styleInstalled: !!document.getElementById("myco-runtime-style"),
            userRole: user?.getAttribute("data-myco-role") === "user",
            assistantRole:
              assistant?.getAttribute("data-myco-role") === "assistant",
            userIdentity:
            ...
            assistantBubble:
              document.getElementById("acceptance-markdown")
                ?.querySelector('[data-myco-prose="assistant"]') != null,
            userBubblePreserved:
              !nativeUserBubble?.hasAttribute("data-myco-prose") &&
              nativeUserBubble?.classList.contains("acceptance-native-user-bubble"),
            avatarCircular:
              avatarStyle?.borderRadius === "50%" &&
              avatarStyle?.objectFit === "cover",
            codePreserved: !code?.hasAttribute("data-myco-prose"),
            toolPreserved: !tool?.hasAttribute("data-myco-prose"),
            diffPreserved: !diff?.hasAttribute("data-myco-prose"),
            statusPreserved: !status?.hasAttribute("data-myco-prose"),
            toolbarPreserved: !toolbar?.hasAttribute("data-myco-prose"),
            nonConversationRegionsClean:
              [header, welcome, composer].every(region =>
                !!region &&
                !region.hasAttribute("data-myco-turn") &&
                !region.querySelector(
                  "[data-myco-turn],.mc-avatar,.mc-nickname"
                ))
          };
        })()
        """;
    ...
    public const string DestroyCheckScript =
        """
        (() => ({
          runtimeStyleRemoved: !document.getElementById("myco-runtime-style"),
          createdNodesRemoved:
            document.querySelectorAll("[data-myco-created=true]").length === 0,
          turnMarkersRemoved:
            document.querySelectorAll("[data-myco-turn]").length === 0,
          proseMarkersRemoved:
            document.querySelectorAll("[data-myco-prose]").length === 0,
          fixtureStillVisible:
            !!document.getElementById("myco-visual-acceptance")
        }))()
        """;
        ...
        (async () => {
          ...
          return {
            hostAttributeApplied:
              document.documentElement.dataset.theme === theme,
            runtimeThemeApplied:
              document.documentElement.getAttribute("data-myco-host-theme") === theme,
            styleSingleton:
              document.querySelectorAll("#myco-runtime-style").length === 1,
    ...
    private const string FixtureTemplate =
        ...
        (() => {
          ...
          document.title = `MyCO Visual Acceptance ${runId}`;
          document.documentElement.dataset.theme = "dark";
          document.documentElement.style.background = "#0f1012";
          document.body.innerHTML = `
            <div id="myco-visual-acceptance">
              <style id="myco-acceptance-fixture-style">
                ...
                html[data-theme="light"] #myco-visual-acceptance {
                  background:
              ...
              <header class="acceptance-header">
                <div>
                  <div class="acceptance-title">MyCO 双实例视觉验收 · Codex B</div>
                ...
                <section data-content-search-turn-key="synthetic-turn-user">
                  <div id="acceptance-user"
                    ...
                    <div id="acceptance-native-user-bubble"
                      ...
                      <p>这是一条合成 User 消息，原生气泡不应被 MyCO 重绘</p>
                    </div>
                  </div>
                </section>
                ...
                <section data-content-search-turn-key="synthetic-turn-assistant">
                  <div id="acceptance-assistant"
                    ...
                    <div id="acceptance-markdown" class="acceptance-markdown markdownContent-acceptance">
                      ...
                      <p>第二段用于检查多段落间距。下面是一段较长文本，用来确认窄窗口和宽窗口下都能自然换行，不会溢出、截断或覆盖旁边的头像：MyCO visual acceptance keeps the official application shell intact while applying a scoped presentation layer only to assistant prose.</p>
                    </div>
          ...
          return {
            ...
            root: !!document.getElementById("myco-visual-acceptance")
          };
        })()
        """;
}
