...
internal static class Program
{
    ...
    private static async Task<int> StartAsync(string[] args)
    {
        ...
        await new VisualAcceptanceStateStore(paths.StateFile).WriteAsync(
            new VisualAcceptanceState
            {
                ...
                StartedAt = startedAt,
                UpdatedAt = startedAt,
                PipeName = $"MyCO.VisualAcceptance.{paths.RunId}"
            }).ConfigureAwait(false);
    ...
    private static Dictionary<string, string> CommandExamples(string runId) =>
        ...
        {
            ["restart"] = $"dotnet MyCO.VisualAcceptance.dll restart --run-id {runId}",
            ["themeLight"] =
                $"dotnet MyCO.VisualAcceptance.dll theme --run-id {runId} --mode light",
            ["themeDark"] =
                $"dotnet MyCO.VisualAcceptance.dll theme --run-id {runId} --mode dark",
            ["status"] = $"dotnet MyCO.VisualAcceptance.dll status --run-id {runId}",
            ["disable"] = $"dotnet MyCO.VisualAcceptance.dll disable --run-id {runId}",
            ["stop"] = $"dotnet MyCO.VisualAcceptance.dll stop --run-id {runId}"
        };
    ...
    private static void WriteHelp()
    {
        Console.WriteLine(
            """
            MyCO.VisualAcceptance (development/acceptance-only)
