...
public sealed class ConfigStore
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var brandNormalized = NormalizeBrandPaths(config);
            if (brandNormalized != config)
            {
                config = brandNormalized;
                migrated = true;
            }
            if (migrated)
            {
    ...
    {
        _paths.EnsureDirectories();
        var normalized = NormalizeBrandPaths(Normalize(config));
    ...
    private AppConfig NormalizeBrandPaths(AppConfig config)
    {
        return config with
        {
            Assistant = config.Assistant with
            {
                Avatar = _paths.MapLegacyPath(config.Assistant.Avatar)
            },
            User = config.User with
            {
                Avatar = _paths.MapLegacyPath(config.User.Avatar)
            }
        };
    }
}
...
internal static class ConfigMigration
{
    ...
    public static ConfigMigrationResult Migrate(JsonObject source, int schemaVersion)
    {
        ...
        {
            throw new InvalidOperationException("Configuration schema is newer than MyCO.");
        }
        ...
        if (migrated["launchCodexOnMycoStart"] is null &&
            migrated["launchCodexOnMyCodexStart"] is JsonNode legacyLaunchValue)
        {
            migrated["launchCodexOnMycoStart"] = legacyLaunchValue.DeepClone();
            changed = true;
        }
        changed |= EnsureValue(
            migrated,
