// Debounces relevant DOM mutations and ignores nodes created by MyCO itself.
  ...
  start(root: Node, refresh: () => void): void {
    ...
    this.observer = new window.MutationObserver((mutations) => {
      const relevant = mutations.some((mutation) => {
        ...
        if (mutationElement?.closest("[data-myco-created=true]")) {
          return false;
        }
        if (mutation.type === "characterData") {
          const parent = mutation.target.parentElement;
          return Boolean(
            parent && !parent.closest("[data-myco-created=true]")
          );
        }
        ...
        return Array.from(mutation.addedNodes).some(
          (node) =>
            ...
            !node.matches("[data-myco-created=true]")
        );
      });
