...
export function stableAttributes(element: Element): Record<string, string> {
  ...
  for (const attribute of Array.from(element.attributes)) {
    if (attribute.name.startsWith("data-myco")) continue;
...
export function isInteractiveOrTool(element: Element): boolean {
  ...
  if (
    element.closest(
      "pre,code,[data-myco-exclude],[role=toolbar],[role=status],[data-testid*=tool],[data-testid*=command],[data-testid*=terminal],[data-testid*=diff],[data-testid*=approval],[data-content-type=tool],[data-content-type=command],[data-content-type=terminal],[data-content-type=diff],[data-content-type=approval],[data-content-type=status]"
    )
  ) {
    return true;
  }
...
export function isInNonConversationRegion(element: Element): boolean {
  return Boolean(
    element.closest(
      `${NON_CONVERSATION_REGION_SELECTOR},[data-myco-created=true]`
    )
  );
}
