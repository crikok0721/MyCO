...
export class CalibrationController {
  ...
  stop(document: Document): void {
    ...
    this.hovered?.removeAttribute("data-myco-inspector");
    for (const sample of this.samples) {
      sample.removeAttribute("data-myco-inspector");
    }
    this.hovered = null;
    this.samples = [];
    this.active = false;
  ...
  private handlePointerOver = (event: Event): void => {
    ...
    if (!this.samples.includes(target)) {
      target.setAttribute("data-myco-inspector", "hover");
    }
  };
  ...
  private handleClick = (event: Event): void => {
    ...
    selected.setAttribute("data-myco-inspector", "selected");
...
const EXCLUDED_CALIBRATION_TARGETS =
  ...
  "[contenteditable=true],[data-myco-created=true]";
