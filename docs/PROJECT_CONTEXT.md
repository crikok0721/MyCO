...
当前增量开发目标为 `0.99.2`，已由 `v0.99.2` 标签生成并发布正式发行包；
真实环境验收门禁仍然保留。本轮代码已切换到配置 schema 7、几何
...
当前增量还修复了 Whole 模式伸缩 Markdown 外壳导致的空白窄气泡、Assistant
短回复固定宽度，并让 Home/Appearance 使用同一动态气泡宽度基准。新配置和
恢复默认通过受管头像服务播种 `assets/MyCO-logo.png`，保留默认昵称“菲叶子”
和已有自定义头像。缺陷索引见 `docs/ERROR_LEDGER.md`；计划与失败测试完成后
自动执行的工作流由 `MEM-004` 生效，旧确认门禁仅作历史证据。

本轮还为预览双方气泡补上共享的主题感知可见边界，并将托盘通知改为每个
MyCO 进程第一次用户主动最小化时优先请求带 64px 蓝色信息标记的原生
ToastGeneric，失败时回退 BalloonTip，成功返回后才提交进程内 claim；真实
WPF 与 Windows Shell 显示仍不能由自动化测试代替。
